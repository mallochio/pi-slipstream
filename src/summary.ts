...
function digestTerminalFinalAnswer(lines: string[]): string {
	...
	return capsuleList(selected.map((line) => compactOneLine(line, 220)), 3);
}
...
function verificationDigest(snapshot: Snapshot): string {
	...
	return capsuleList(lines.map((line) => compactOneLine(line, 220)), 3);
}
...
export function buildCurrentStateCapsule(
	...
	const manifest = snapshot.manifest;
	const allArtifactRefs = uniqueNonEmpty([...artifactRefs, ...manifest.artifactRefs]);
	const fileCount = manifest.filesModified.length + manifest.filesDeleted.length;
...
${capsuleList(manifest.latestExchangeState.map((line) => compactOneLine(line, 220)), 2)}
...
Trust-hygiene rule: separate current facts from historical evidence before writing. Include the latest actionable ask, active approved scope, explicitly deferred/not-approved work, and stale branches in the narrative when they affect the next action. Redact secret-shaped values; never print API keys, tokens, passwords, private keys, certificates, or bearer values. For auth/cert/key/deletion/deploy state, do not infer: either preserve exact current evidence, mark unknown/unverified, or make recheck the next action.
