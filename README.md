...
| `minContinuationTurns`  |                    `1` | Minimum continuation turns before auto validation.                                                                                                                                                                                                                                                                                             |
| `maxContinuationTurns`  |                    `4` | Maximum continuation turns collected for auto validation.                                                                                                                                                                                                                                                                                      |
...
| `turn_end`                           | At `triggerContextPercent`, starts candidate generation after final assistant responses, latches compaction urgency, revalidates stale pending summaries against the live branch head with a current retained-tail boundary, and activates only fresh pending summaries with `ctx.compact()`. |
...
Routine progress stays in the widget/status line. Interactive rejected-summary decisions use the confirmation/select UI when `rejectedSummaryMode` is `"ask"`; non-interactive accepted/rejected outcomes are recorded in compaction details or a concise warning instead of chat-style progress spam.
...
- 214 Node test-runner tests passed.
- `tsc --noEmit` passed.
...
| Auto live Pi flow                              | Auto prepared at final assistant boundary; second turn supplied continuation evidence; adoption preserved expected state.                                                                                                     |
